...
% FIM 분석
correlationFIM = false;                  % false | 'A-opt' | 'D-opt' | 'E-opt' | 'Cond'
...
interpInterval = 100;                   % 경로 보간 간격 (0.5m 단위로 위치 생성)      
...
for inumNode = 1:numNodePositions
    ...
    if validLocatorsCount >= 2 && ~all(angleEstGlobal(:,1) == angleEstGlobal(1,1))
        ...
        if strcmp(estimationMethod, "Non-linear")
            ...
            posNodeEst(:, inumNode) = nonLinearTriangulation(posLin, posActiveLocators(:, validIdx), angleEstGlobal(validIdx, :));
        else
            % 선형 측정 결과 그대로 사용
